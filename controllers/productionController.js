...
const getAllBatches = async (req, res) => {
    const { seedling_id, production_type, status, from_date, to_date, page = 1, limit = 20 } = req.query;
    const offset = (page - 1) * limit;
    const params = [];
    const conditions = [];

    if (seedling_id) { params.push(seedling_id); conditions.push(`pb.seedling_id = $${params.length}`); }
    ...
    if (from_date) { params.push(from_date); conditions.push(`pb.created_at >= $${params.length}`); }
    if (to_date) { params.push(to_date); conditions.push(`pb.created_at <= $${params.length}`); }

    const where = conditions.length ? 'WHERE ' + conditions.join(' AND ') : '';

    try {
        const countResult = await db.query(`SELECT COUNT(*) FROM production_batches pb ${where}`, params);
...
const createSeedBatch = async (req, res) => {
    const {
        seedling_id, seed_source, seed_quantity,
        sowing_date, germination_date, germination_percent,
        produced_quantity, success_quantity, failed_quantity, remarks
    } = req.body;

    if (!seedling_id || !sowing_date || !produced_quantity) {
        return res.status(400).json({ success: false, message: 'চারা, বপন তারিখ ও উৎপাদিত পরিমাণ দিন।' });
    }
    ...
    try {
        ...
        const autoGermPercent = germination_percent ||
            (seed_quantity > 0
                ? parseFloat(((produced_quantity / seed_quantity) * 100).toFixed(2))
                : parseFloat(((produced_quantity / (produced_quantity + (failed_quantity || 0))) * 100).toFixed(2)) || null);
        ...
        const batchResult = await client.query(
            `INSERT INTO production_batches
             ...
             RETURNING *`,
            [seedling_id, seed_source, seed_quantity, sowing_date, germination_date,
             autoGermPercent, produced_quantity, success_quantity || produced_quantity,
             failed_quantity || 0, remarks, req.user.id]
        );
        ...
        const stockResult = await client.query(
            `SELECT COALESCE(current_stock, 0) FROM seedlings WHERE id = $1`, [seedling_id]
        );
        ...
        await client.query(
            ...
            [seedling_id, batch.id, produced_quantity, newBalance,
             `ব্যাচ ${batch.batch_code} থেকে উৎপাদন`, req.user.id]
        );
        ...
        res.status(201).json({ success: true, message: 'বীজ উৎপাদন ব্যাচ তৈরি হয়েছে।', data: batch });
    } catch (err) {
...
const createAsexualBatch = async (req, res) => {
    const {
        seedling_id, production_type, mother_plant_id, rootstock,
        scion_variety, propagation_date, produced_quantity,
        success_quantity, failed_quantity, remarks
    } = req.body;

    if (!seedling_id || !production_type || !propagation_date || !produced_quantity) {
        return res.status(400).json({ success: false, message: 'সব তথ্য পূরণ করুন।' });
    }

    const successQty = success_quantity || 0;
    const successPercent = produced_quantity > 0 ? (successQty / produced_quantity * 100).toFixed(2) : 0;
    ...
    try {
        ...
        const batchResult = await client.query(
            `INSERT INTO production_batches
             ...
             RETURNING *`,
            [seedling_id, production_type, mother_plant_id, rootstock, scion_variety,
             propagation_date, produced_quantity, successQty, failed_quantity || 0,
             successPercent, remarks, req.user.id]
        );
        ...
        await client.query(
            `INSERT INTO stock_transactions
             (seedling_id, batch_id, txn_type, quantity, direction, balance_after, notes, created_by)
             VALUES ($1,$2,'production',$3,'+',$4,$5,$6)`,
            [seedling_id, batch.id, successQty, newBalance,
             `ব্যাচ ${batch.batch_code} থেকে ${production_type} উৎপাদন`, req.user.id]
        );
        ...
        await client.query('COMMIT');
        res.status(201).json({ success: true, message: 'অঙ্গজ বংশবিস্তার ব্যাচ তৈরি হয়েছে।', data: batch });
    } catch (err) {
        await client.query('ROLLBACK');
        res.status(500).json({ success: false, error: err.message });
    } finally {
...
module.exports = { getAllBatches, createSeedBatch, createAsexualBatch, getBatchById };
