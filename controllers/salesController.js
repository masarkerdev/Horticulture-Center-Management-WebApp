...
const getAllSales = async (req, res) => {
    const { from_date, to_date, payment_status, search, page = 1, limit = 20 } = req.query;
    ...
    if (search) {
        params.push(`%${search}%`);
        conditions.push(`(s.invoice_no ILIKE $${params.length} OR s.customer_name ILIKE $${params.length})`);
    }
...
const getSaleById = async (req, res) => {
    try {
        ...
        if (sale.rows.length === 0) {
            return res.status(404).json({ success: false, message: 'চালান পাওয়া যায়নি।' });
        }

        const items = await db.query(
            `SELECT si.*, s.name_bn, s.name_en, s.seedling_code, pb.batch_code
             FROM sales_items si
...
const createSale = async (req, res) => {
    const {
        ...
        sale_date, items, discount, payment_method, payment_status, notes
    } = req.body;

    if (!items || items.length === 0) {
        return res.status(400).json({ success: false, message: 'কমপক্ষে একটি আইটেম দিন।' });
    }
    ...
    try {
        ...
        const saleResult = await client.query(
            ...
            [customer_id, customer_name, customer_phone, customer_address,
             ...
             subtotal, discount || 0, totalAmount,
             payment_method || 'cash', payment_status || 'paid', notes, req.user.id]
        );
        ...
        for (const item of items) {
            ...
            await client.query(
                ...
                [item.seedling_id, item.batch_id || null, item.quantity, newStock,
                 sale.id, `চালান ${sale.invoice_no} থেকে বিক্রয়`, req.user.id]
            );
        }
...
module.exports = { getAllSales, getSaleById, createSale, getTodaySummary, getMonthlySales };
